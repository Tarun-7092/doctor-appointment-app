...
const updateProfile = async (req, res) => {
    ...
    try {
        ...
        await userModel.findByIdAndUpdate(req.userId, { name, phone, address: address ? JSON.parse(address) : {}, dob, gender })
...
const createCheckoutSession = async (req, res) => {
  try {
      ...
      line_items: [
        {
          price_data: {
            ...
            unit_amount: appointment.amount * 100, // Stripe uses smallest unit
          },
          quantity: 1,
        },
      ],
    ...
    // 3️⃣ Send session URL to frontend
    res.json({
      success: true,
      url: session.url,
    });
...
const verifyPayment = async (req, res) => {
  try {
    ...
    // 3️⃣ Get appointmentId from metadata
...
export { registerUser, loginUser, getProfile, updateProfile, bookAppointment, listAppointment, cancelAppointments, createCheckoutSession, verifyPayment }